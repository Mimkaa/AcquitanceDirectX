...
float4 main(float3 WorldPos : Position, float3 normal : Normal, uint tid : SV_PrimitiveID) : SV_Target
{
    ...
    const float3 s = attenuation * (light_diffuse * attIntensity) * specularIntensity * pow(max(0.0f, dot(normalize(-r), normalize(WorldPos))), specularPower);
    
    
    return float4(saturate(material_color[(tid / 2) % 6] * (d + light_ambient + s)), 1.0f);
