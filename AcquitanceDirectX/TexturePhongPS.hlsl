...
float4 main(float3 WorldPos : Position, float3 normal : Normal, float2 tc : TexCoords) : SV_Target
{
    ...
    return float4(saturate(tex.Sample(splr, tc) * (d + light_ambient + s)),1.0f);
